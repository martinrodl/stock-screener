...
const CashFlowStatements = ({ symbol }) => {
            ...
            {statements.length > 0 ? (
                <table className="min-w-full bg-white border">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td className="py-2 px-4 border-b">Investing Cash Flow</td>
                            {statements.map((statement) => (
                                <td key={statement._id} className="py-2 px-4 border-b">
                                    {statement.netCashUsedForInvestingActivites?.toLocaleString() ||
                                        '-'}
                                </td>
                            ))}
                        </tr>
                        <tr>
                            <td className="py-2 px-4 border-b">Financing Cash Flow</td>
                            {statements.map((statement) => (
                                <td key={statement._id} className="py-2 px-4 border-b">
                                    {statement.netCashUsedProvidedByFinancingActivities?.toLocaleString() ||
                                        '-'}
                                </td>
                            ))}
                        </tr>
                        <tr>
                            <td className="py-2 px-4 border-b">Net Cash Flow</td>
                            {statements.map((statement) => (
                                <td key={statement._id} className="py-2 px-4 border-b">
                                    {statement.netChangeInCash?.toLocaleString() || '-'}
                                </td>
                            ))}
                        </tr>
                        <tr>
