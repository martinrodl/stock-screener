...
export const updateGrowthCashflowMetrics = async (stockSymbol, limit = 10) => {
  try {
    const cashflowMetrics = await getStockCashflowgrowthMetric(
      stockSymbol,
      limit
    );
    const stock = await Stock.findOne({ symbol: stockSymbol });
    ...
    for (const metric of cashflowMetrics) {
      const dateExists = stock.growthCashflowtMetrics.some(
        (gm) => gm.date === metric.date
      );
...
export const updateGrowthIncomegrowthmetrics = async (
  stockSymbol,
  limit = 10
) => {
  try {
    ...
    console.log("Updating growth metrics of ", stock.symbol);
    for (const metric of keyMetrics) {
      const dateExists = stock.growthIncomeMetrics.some(
        (gm) => gm.date === metric.date
      );
...
export const updateIncomeStatements = async (stockSymbol, limit = 10) => {
  try {
    ...
    for (const statement of incomeStatements) {
      const dateExists = stock.incomeStatements.some(
        (doc) => doc.date === statement.date
      );
...
export const updateCashflowStatements = async (stockSymbol, limit = 10) => {
  try {
    ...
    for (const statement of cashflowStatements) {
      const dateExists = stock.cashflowStatements.some(
        (doc) => doc.date === statement.date
      );
